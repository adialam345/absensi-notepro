    ...
    <div class="bg-[#ff040c] p-3 text-white">
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-2">
                ...
                <span class="font-semibold text-sm">Edit Karyawan</span>
            </div>
        </div>
    </div>
    ...
    <div class="p-4">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-sm p-6">
                <h2 class="text-lg font-semibold text-gray-800 mb-6">Edit Data Karyawan</h2>
                ...
                <form action="{{ route('admin.karyawan.update', $karyawan->id) }}" method="POST">
                        ...
                        <div>
                            ...
                            <select name="lokasi_kantor_id" required 
                                ...
                                @foreach($lokasiKantor as $lokasi)
                                    <option value="{{ $lokasi->id }}" {{ old('lokasi_kantor_id', $karyawan->lokasi_kantor_id) == $lokasi->id ? 'selected' : '' }}>
                                        {{ $lokasi->nama_lokasi }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        <div>
                            ...
                            <input type="time" name="jam_kerja_masuk" value="{{ old('jam_kerja_masuk', $karyawan->jam_masuk) }}" required 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#ff040c]">
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Jam Kerja Pulang</label>
                            <input type="time" name="jam_kerja_pulang" value="{{ old('jam_kerja_pulang', $karyawan->jam_pulang) }}" required 
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#ff040c]">
                        </div>
                    </div>
